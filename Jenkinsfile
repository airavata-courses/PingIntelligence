node {
    ...
    stage('Build Docker Image'){    		
       app = docker.build("pingintelligence/consumer-queue-image-1")    
    }
    ...
    stage('Deploying new service into Kubernetes') {
        sshagent(credentials: ['ssh-key-iu']) {
            ...
            sh '''
                ...
                sudo su
                sudo kubectl delete deployment,service consumer-queue-1 &&
                sudo kubectl apply -f consumer_queue2.yaml"
               '''
        }
