...
app.post("/login", (req, res) => {
  ...
  axios
    .post("http://user-mgmt:8080/user/login", {
...
app.post("/register", (req, res) => {
  console.log(req.body);
  axios
    .post("http://user-mgmt:8080/user/signup", {
      username: req.body.username,
      password: req.body.password,
...
app.post("/verifyusertoken", (req, res) => {
  ...
  axios
    .post(
      "http://user-mgmt:8080/user/verify",
      qs.stringify({
...
app.post("/createalbum", (req, res) => {
  console.log(req.body);
  axios
    .post("http://localhost:8090/album/create", {
      name: req.body.name,
...
app.post("/useralbums", (req, res) => {
  // console.log(req.body)
  axios
    .get("http://localhost:8090/album/allalbums?username=" + req.body.username)
    .then(function (response) {
      console.log(response.data);
      res.send(response.data);
    })
...
app.post("/uploadalbumphotos", (req, res) => {
  ...
  for (i = 0; i < data.length; i++) {
    fs.writeFileSync("./testfile1.jpg", data.data, (err) => {
      if (err) throw err;
    ...
    var options = {
      ...
      formData: {
        ...
        files: fs.createReadStream("testfile1.jpg"),
      },
    };
...
app.post("/findonealbum", (req, res) => {
  var options = {
    method: "GET",
    url: "http://localhost:8090/album/locatealbum?albumname=" +
    req.body.albumname 
  };
  console.log(options)
  request(options, function (error, response, body) {
